  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <div className="absolute text-center">
        ...
        <span className="text-[10px] text-slate-400 font-semibold block uppercase leading-none mt-0.5">Skor</span>
      </div>
    </div>
  );
}
          ...
          <Card className="border-slate-200/80 bg-white">
            ...
            <CardContent className="space-y-3 text-sm text-slate-600">
              <div className="p-3.5 bg-slate-50 rounded-lg space-y-1 border border-slate-100">
                <span className="font-semibold text-slate-800">1. Pilar Keaktifan (40%)</span>
                <p className="text-xs text-slate-500 leading-relaxed">
                  Rasio anggota aktif vs total terdaftar. Menguji komitmen gotong royong internal.
                </p>
              </div>
              <div className="p-3.5 bg-slate-50 rounded-lg space-y-1 border border-slate-100">
                <span className="font-semibold text-slate-800">2. Pilar Keuangan (30%)</span>
                <p className="text-xs text-slate-500 leading-relaxed">
                  Rasio pendapatan tahunan dibanding koperasi terbaik dalam database nasional.
                </p>
              </div>
              <div className="p-3.5 bg-slate-50 rounded-lg space-y-1 border border-slate-100">
                <span className="font-semibold text-slate-800">3. Pilar Pasokan (30%)</span>
                <p className="text-xs text-slate-500 leading-relaxed">
              ...
              <div className="border-t border-slate-100 pt-3">
                <span className="font-semibold text-slate-700 block mb-2 text-xs uppercase tracking-wider">Grade Kemitraan:</span>
            ...
            <Card className="lg:col-span-2 border-slate-200/80 bg-white relative overflow-hidden">
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <span className="text-[10px] bg-brand-navy text-white px-2.5 py-0.5 rounded-full font-semibold uppercase">
                      {selectedCoop.province}
                    </span>
                    <CardTitle className="text-xl font-semibold text-slate-900 mt-2">
                      {selectedCoop.name}
                    </CardTitle>
                  ...
                  <div className="flex flex-col items-end">
                    <span className="text-[10px] text-slate-400 font-semibold uppercase mb-1">Grade</span>
                    <span className={`inline-block font-semibold text-2xl px-4 py-1.5 rounded-xl text-white ${grade === 'A' ? 'bg-emerald-500' :
                        grade === 'B' ? 'bg-blue-500' :
              ...
              <CardContent className="pt-6 space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 justify-items-center bg-slate-50 p-5 rounded-xl border border-slate-100">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-[10px] font-semibold text-slate-500 text-center uppercase tracking-wider">Skor Akhir</span>
                  </div>
                  <div className="flex flex-col items-center gap-2">
                    <ProgressRing value={score?.health_score || 0} size={90} strokeWidth={8} colorClass="stroke-emerald-500" />
                    <span className="text-[10px] font-semibold text-slate-500 text-center uppercase tracking-wider">Keaktifan</span>
                  </div>
                  <div className="flex flex-col items-center gap-2">
                    <ProgressRing value={score?.growth_score || 0} size={90} strokeWidth={8} colorClass="stroke-blue-500" />
                    <span className="text-[10px] font-semibold text-slate-500 text-center uppercase tracking-wider">Keuangan</span>
                  </div>
                  <div className="flex flex-col items-center gap-2">
                    <ProgressRing value={score?.supply_score || 0} size={90} strokeWidth={8} colorClass="stroke-brand-orange" />
                    <span className="text-[10px] font-semibold text-slate-500 text-center uppercase tracking-wider">Pasokan</span>
                  </div>
                </div>
                ...
                <div className="grid sm:grid-cols-3 gap-4 text-sm text-slate-600 border-t border-b border-slate-100 py-4">
                  <div className="space-y-1">
                    <span className="text-[10px] text-slate-400 block uppercase font-semibold tracking-wider">Partisipasi Anggota</span>
                    <span className="text-slate-800 font-semibold flex items-center gap-1.5">
                      <Users className="h-4 w-4 text-slate-400" /> {selectedCoop.active_members} Aktif / {selectedCoop.member_count} Total
                    </span>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[10px] text-slate-400 block uppercase font-semibold tracking-wider">Pendapatan Tahunan</span>
                    <span className="text-slate-800 font-semibold flex items-center gap-1.5">
                      <TrendingUp className="h-4 w-4 text-slate-400" /> Rp {selectedCoop.annual_revenue.toLocaleString('id-ID')}
                    </span>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[10px] text-slate-400 block uppercase font-semibold tracking-wider">Komoditas Utama</span>
                  ...
                  {filteredRankings.map((coop) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`cursor-pointer transition-colors duration-150 hover:bg-slate-50/50 ${isSelected ? 'bg-brand-cream/40 font-semibold' : ''
                          }`}
                      >
                        ...
                        <td className="p-4">
                          <span className="font-semibold text-slate-800 block text-sm">{coop.name}</span>
                        ...
                        <td className="p-4 text-center font-semibold text-brand-navy text-sm">
                          {score?.final_score || 0}
                        </td>
                        <td className="p-4 text-center">
                          <span className={`inline-block font-semibold text-[10px] px-2.5 py-1 rounded-full text-white ${grade === 'A' ? 'bg-emerald-500' :
                              grade === 'B' ? 'bg-blue-500' :
                                grade === 'C' ? 'bg-amber-500' : 'bg-red-500'
                            }`}>
                            {grade}
